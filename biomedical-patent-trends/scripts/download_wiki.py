...
def api_key() -> str:
    value = os.environ.get("SCIMINER_API_KEY")
    if isinstance(value, str) and value.strip():
        return value.strip()
    raise SystemExit(
        "SCIMINER_API_KEY is required but was not injected by the SciMiner gateway. "
        "Do not provide an inline key or use another credential source."
    )
