...
- Resolve every SciMiner capability from the live tool-doc index at `https://sciminer.simm.ac.cn/tool_api_files/` and read the selected `*_api_doc.md` immediately before invocation. That document is authoritative for provider/tool names, parameters, upload behavior, polling, and results.
