...
14. Poll the task result and return the `history_url` in the final user-facing
    summary.
...
- Important: when summarizing results to users, attach the `history_url` links of
  every successful task at the end so that users can view the online results,
  rather than showing file download links.
- For long-running tasks without a fixed ETA, poll for no more than 1800
  seconds; if the task is still running, stop polling and return the
  `history_url` so the user can check later.