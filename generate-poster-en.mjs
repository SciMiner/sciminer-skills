...
const replacements = new Map([
  ...
  ['OpenClaw 可从技能商店安装或添加本仓库；Claude Code 可请求安装 sciminer-skills。所有 skill 都要求 SciMiner API key，并统一从 <code>~/.config/sciminer/credentials.json</code> 读取。', 'OpenClaw can install from the skill marketplace or add this repository. Claude Code can install sciminer-skills on request. Every skill requires a SciMiner API key, read consistently from <code>~/.config/sciminer/credentials.json</code>.'],
  ['凭证文件', 'Credential File'],
  ['从 SciMiner 获取免费 API key 后，将其保存到 <strong>~/.config/sciminer/credentials.json</strong> 的 <strong>api_key</strong> 字段中。调用时以 <strong>X-Auth-Token</strong> header 发送。', 'Get a free SciMiner API key, then store it in the <strong>api_key</strong> field of <strong>~/.config/sciminer/credentials.json</strong>. Calls send it as the <strong>X-Auth-Token</strong> header.'],
