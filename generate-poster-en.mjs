...
const replacements = new Map([
  ...
  ['个统一凭证：~/.config/sciminer/credentials.json', 'shared credential: ~/.config/sciminer/credentials.json'],
  ...
  ['OpenClaw 可从技能商店安装或添加本仓库；Claude Code 可请求安装 sciminer-skills。依赖 SciMiner API 的 skill 需先配置凭证文件。', 'OpenClaw can install from the skill marketplace or add this repository. Claude Code can install sciminer-skills on request. Skills that use the SciMiner API require a credential file.'],
  ['凭证文件', 'Credential File'],
  ['从 SciMiner 获取免费 API key 后，保存到 <strong>~/.config/sciminer/credentials.json</strong> 的 <strong>api_key</strong> 字段。调用时以 <strong>X-Auth-Token</strong> header 发送。', 'Get a free SciMiner API key, then store it in <strong>~/.config/sciminer/credentials.json</strong> with an <strong>api_key</strong> field. Calls send that value as the <strong>X-Auth-Token</strong> header.'],
