...
const replacements = new Map([
  ...
  ['个统一凭证文件：~/.config/sciminer/credentials.json', 'shared credential file: ~/.config/sciminer/credentials.json'],
  ...
  ['OpenClaw 可从技能商店安装或添加本仓库；Claude Code 可请求安装 sciminer-skills。所有 SciMiner skill 都从 <code>~/.config/sciminer/credentials.json</code> 读取 API key。', 'OpenClaw can install from the skill marketplace or add this repository. Claude Code can install sciminer-skills on request. All SciMiner skills read the API key from <code>~/.config/sciminer/credentials.json</code>.'],
  ['凭证文件', 'Credential File'],
  ['从 SciMiner 获取免费 API key 后，将其写入 <strong>~/.config/sciminer/credentials.json</strong> 的 <strong>api_key</strong> 字段。调用时以 <strong>X-Auth-Token</strong> header 发送；若文件缺失或字段为空，应停止并提示用户先完成配置。', 'Get a free SciMiner API key, then write it to the <strong>api_key</strong> field in <strong>~/.config/sciminer/credentials.json</strong>. Calls send it as the <strong>X-Auth-Token</strong> header; if the file or field is missing, stop and ask the user to configure it first.'],
