...
```bash
# Global patents (requires free Lens API key)
curl -X POST https://mcp.sciminer.tech/tools/unified/mcp \
  -H "Content-Type: application/json" -H "Accept: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"lens_search_patents","arguments":{"query":"osimertinib EGFR T790M","api_key":"YOUR_LENS_KEY","max_results":20}},"id":8}'

# US patents only (no key needed)
curl -X POST https://mcp.sciminer.tech/tools/unified/mcp \
  -H "Content-Type: application/json" -H "Accept: application/json" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"uspto_ppubs_search_patents","arguments":{"query":"osimertinib EGFR inhibitor","max_results":20}},"id":9}'
```
