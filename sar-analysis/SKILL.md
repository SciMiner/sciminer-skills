...
2. Read the corresponding Markdown file from `https://sciminer.simm.ac.cn/tool_api_files/`.
...
- If `~/.config/sciminer/credentials.json` is missing or does not contain an `api_key` field, stop and tell the user to obtain a free SciMiner API key from `https://sciminer.simm.ac.cn/utility` and store it in that file. Do not try to complete the task by switching to other tools or services.
