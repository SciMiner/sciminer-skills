...
The published Markdown files under `https://sciminer.simm.ac.cn/tool_api_files/` are the single source of truth for the selected peptide-design tool's `provider_name`, `tool_name`, allowed `parameters`, file-upload behavior, request encoding, and submission flow.
